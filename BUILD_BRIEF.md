...
Post-MVP rendering hardening also preserves Markdown title/author metadata, removes authored HTML comments with Micromark-aware code-context and valid-YAML protection, disables raw attributed HTML, supports browser PDF figures, and maps explicitly authored parent-relative resources through opaque per-render allowlist URLs. A bundled lightweight Pandoc filter resolves exact document-local Quarto (`fig-`) and `pandoc-crossref` (`fig:`) figure references without running Quarto or code. One-shot and watch pages use nonce-only script policies; watch responses receive fresh nonces, resource routes serve only supported browser media, and a content-hashed single-flight queue collapses saves that arrive during rendering. Local rewrites retain authored query strings and fragments, raw or encoded UNC and Windows device paths are rejected before accessing the file system, and shutdown closes the server while cancelling an active Pandoc render. The public resource endpoint still rejects arbitrary traversal and symlink escape.

The `0.3.0` release adds display-only `[an: ...]` highlighting for Markdown/QMD, matching the note-only appearance used in pi-markdown-preview and Pi Studio. A bracket-aware scanner adapted from the reference implementation uses Micromark code/prose boundaries and preserves valid YAML front matter. Unlike the reference browser-placeholder pipeline, it emits native Pandoc spans so Pandoc handles inline formatting, links, math, HTML escaping, and stable heading IDs without additional browser JavaScript. Existing raw-HTML restrictions and CSP remain unchanged. Notes wrap in light/dark one-shot and watch previews. Their soft backgrounds and borders follow the theme accent (blue), with normal text colour and a stronger tint in dark mode; the surrounding document theme is unchanged. Source files, code examples, escaped/incomplete markers, and standalone LaTeX are not rewritten. This is deliberately not a commenting/editor/AI workflow. `test/fixtures/annotations.qmd` provides a self-contained manual smoke document. Publication remains a separate owner-approved release operation.

The `0.3.1` development milestone adapts pi-markdown-preview's safe bare `<sup>`/`<sub>` compatibility to TypeScript. Plain single-line tag contents become Pandoc superscript/subscript notation; raw HTML and raw attributed blocks remain disabled. Code/escape/link-attribute/YAML/math contexts are protected, and tag contents cannot introduce citations, links, annotations, or formatting. Existing annotation math/attribute scanners are shared rather than duplicated. One-shot/watch and literal/security regressions cover the feature; `test/fixtures/superscript.qmd` is the manual smoke fixture. No Pi runtime dependency or source-file changes are introduced.
